...
-(void)viewWillAppear:(BOOL)animated{
    ...
    @try {
        ...
        NSArray *creditCards = [NSArray arrayWithArray:[mainDelegate getAllCreditCardsForCurrentCustomer]];
        
        if ([creditCards count] > 0) {
            self.isPayment = NO;
            self.nextButton.text = @"Register";
    ...
    @try {
        ...
        if ([status isEqualToString:@"success"]) {
            ...
            if (self.isPayment) {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Sucess!" message:@"You have registered successfully!" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
                [alert show];
                [self.navigationController popViewControllerAnimated:YES];
            }
